<nav-header inline-template>
    <div>
        <div class="bg-blue-dark">
            <div class="container px-4 mx-auto">
                <div class="flex items-center py-4 md:justify-between">
                    ...
                    <div class="items-center ml-6 text-right md:w-auto md:flex">
                        @if (Route::has('login'))
                        @auth
                        <div class="relative group" @click="showSubMenu = !showSubMenu">
                            <div class="flex items-center px-2 py-1 text-sm text-white cursor-pointer">
                                <div>
                                    <img class="inline-block w-8 h-8 rounded-full"
                                        src="{{ auth()->user()->gravatar_link }}" alt="">
                                </div>
                                <div class="hidden ml-2 md:flex md:items-center">
                                    <span class="mr-1 text-sm text-white">{{ auth()->user()->name }}</span>
                                    <div>
                                        <svg class="block w-4 h-4 text-white opacity-50 fill-current"
                                            xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                            <path
                            ...
                            <div v-cloak class="absolute z-10 items-center w-full pt-4 bg-blue-dark"
                                :class="showSubMenu ? 'visible' : 'invisible'">
                                <a href="{{ route('dashboard') }}"
                                    class="block px-3 py-2 text-white hover:text-gray-800 hover:bg-blue-500">Панель
                                    управл.</a>
                                @if(auth()->user()->is_admin)
                                <a href="{{ route('admin.enter-data.index') }}"
                                    class="block px-3 py-2 text-white hover:text-gray-800 hover:bg-blue-500">Ввод
                                    данных</a>
                                @endif
                                <hr class="mx-2 border-t border-grey-light">
                                <a href="{{ route('logout') }}"
                                    class="block px-3 py-2 text-white hover:text-gray-800 hover:bg-blue-500" onclick="event.preventDefault();
        ...
        <div class="block bg-white border-b md:block bg-blue-dark">
            <div class="container px-4 mx-auto">
                <div class="flex justify-between">
                    <div class="flex">
                        <div class="flex mr-4 -mb-px">
                            <a href="{{ route('index') }}"
                                class="no-underline {{ (request()->routeIs('statistics.teams') || request()->routeIs('index')) ? 'text-blue-500 md:hover:border-blue-500' : 'text-gray-500 opacity-100 md:hover:border-gray-500' }} flex items-center py-4 border-b border-transparent hover:opacity-100">
                                <svg class="w-6 h-6 mr-2 fill-current" xmlns="http://www.w3.org/2000/svg"
                                    viewBox="0 0 20 20">
                                    <path
                    ...
                    <div class="flex items-center pl-4">
                        <div id="search-toggle" @click="showSearch=!showSearch" v-show="!isComparing"
                            class="search-icon cursor-pointer pl-6 mr-2 {{(request()->routeIs('statistics.teams') || request()->routeIs('index')) ? '' : 'hidden' }}">
                            <svg class="inline w-4 h-4 pointer-events-none fill-current text-grey-darkest"
                                xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                <path
                        ...
                        class="relative group cursor-pointer pl-2 {{(request()->routeIs('statistics.teams') || request()->routeIs('index')) ? '' : 'hidden' }}" 
                        v-show="!isComparing">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"
                                class="w-4 h-4 pointer-events-none fill-current text-grey-darkest">
                            ...
                            <div v-cloak class="absolute right-0 z-10 pt-4 bg-white"
                                style="width:8em;"
        ...
        <div class="relative w-full bg-white shadow-xl" id="search-content" v-cloak>
            <div v-if="showSearch" class="container p-4 mx-auto text-black">
                <input id="searchfield" type="search" placeholder="Поиск..." :autofocus="showSearch"
                    v-model="searchText"
                    class="w-full text-lg leading-normal text-gray-800 transition appearance-none focus:outline-none focus:border-transparent">
            </div>
        </div>
    </div>
</nav-header>