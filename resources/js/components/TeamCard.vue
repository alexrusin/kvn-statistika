<template>
 <div class="w-full px-1 my-2 md:w-1/2 lg:my-4 lg:px-4 lg:w-1/3">
    ...
    <div v-else class="overflow-hidden bg-white rounded-lg shadow-lg">
        <div class="relative cursor-pointer pb-2/3 card-image" @click="teamClicked">
            <img class="object-cover w-full h-full" :class="selected ? 'opacity-50' : 'opacity-100'" :src="team.image_url" :alt="team.name">
            <svg v-if="selected" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" class="absolute w-6 text-gray-800 fill-current position-check">
            ...
            <svg v-if="!selected" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" class="absolute w-8 position-check">
               <rect width="10" height="10" style="fill:rgba(255,255,255,.75);stroke-width:1;stroke:rgb(21,22,25)" />
            </svg>
        </div>
        <div class="p-6">
            ...
            <h4 class="mt-1 text-lg font-semibold leading-tight truncate">{{ team.name }}</h4>
            <div class="relative mt-1" >
                ОКГ: {{ this.okg }} 
                <span class="text-sm text-gray-600"> / игру </span>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" class="inline w-4 ml-1 text-teal-700 cursor-pointer fill-current" @click="toggleTooltip('showOkgTooltip')">
            ...
            <div class="relative mt-1">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" class="inline w-4 ml-1 text-teal-700 cursor-pointer fill-current" @click="toggleTooltip('showEfficiencyTooltip')">
                    <path d="M10 .4A9.6 9.6 0 0 0 .4 10a9.6 9.6 0 1 0 19.2-.001C19.6 4.698 15.301.4 10 .4zm-.151 15.199h-.051c-.782-.023-1.334-.6-1.311-1.371.022-.758.587-1.309 1.343-1.309l.046.002c.804.023 1.35.594 1.327 1.387-.023.76-.578 1.291-1.354 1.291zm3.291-6.531c-.184.26-.588.586-1.098.983l-.562.387c-.308.24-.494.467-.563.688-.056.174-.082.221-.087.576v.09H8.685l.006-.182c.027-.744.045-1.184.354-1.547.485-.568 1.555-1.258 1.6-1.287a1.65 1.65 0 0 0 .379-.387c.225-.311.324-.555.324-.793 0-.334-.098-.643-.293-.916-.188-.266-.545-.398-1.061-.398-.512 0-.863.162-1.072.496-.216.341-.325.7-.325 1.067v.092H6.386l.004-.096c.057-1.353.541-2.328 1.435-2.897.563-.361 1.264-.544 2.081-.544 1.068 0 1.972.26 2.682.772.721.519 1.086 1.297 1.086 2.311-.001.567-.18 1.1-.534 1.585z"/>
                </svg>
            ...
            <div class="relative mt-1">
                Индекс Белого: {{ this.whiteIndex }}
                 <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" class="inline w-4 ml-1 text-teal-700 cursor-pointer fill-current" @click="toggleTooltip('showWhiteTooltip')">
                    <path d="M10 .4A9.6 9.6 0 0 0 .4 10a9.6 9.6 0 1 0 19.2-.001C19.6 4.698 15.301.4 10 .4zm-.151 15.199h-.051c-.782-.023-1.334-.6-1.311-1.371.022-.758.587-1.309 1.343-1.309l.046.002c.804.023 1.35.594 1.327 1.387-.023.76-.578 1.291-1.354 1.291zm3.291-6.531c-.184.26-.588.586-1.098.983l-.562.387c-.308.24-.494.467-.563.688-.056.174-.082.221-.087.576v.09H8.685l.006-.182c.027-.744.045-1.184.354-1.547.485-.568 1.555-1.258 1.6-1.287a1.65 1.65 0 0 0 .379-.387c.225-.311.324-.555.324-.793 0-.334-.098-.643-.293-.916-.188-.266-.545-.398-1.061-.398-.512 0-.863.162-1.072.496-.216.341-.325.7-.325 1.067v.092H6.386l.004-.096c.057-1.353.541-2.328 1.435-2.897.563-.361 1.264-.544 2.081-.544 1.068 0 1.972.26 2.682.772.721.519 1.086 1.297 1.086 2.311-.001.567-.18 1.1-.534 1.585z"/>
                </svg>
                <tooltip transition="fade" :show="showWhiteTooltip">Равномерность распределения шуток среди участников одной команды. 6 - линия стендапа</tooltip>
            </div>
             <div class="relative mt-1">
                Народная оценка: {{ this.peoplesPoints }}
                 <span class="text-sm text-gray-600"> / игру </span>
                 <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" class="inline w-4 ml-1 text-teal-700 cursor-pointer fill-current" @click="toggleTooltip('showPeoplesPointsTooltip')">
                    <path d="M10 .4A9.6 9.6 0 0 0 .4 10a9.6 9.6 0 1 0 19.2-.001C19.6 4.698 15.301.4 10 .4zm-.151 15.199h-.051c-.782-.023-1.334-.6-1.311-1.371.022-.758.587-1.309 1.343-1.309l.046.002c.804.023 1.35.594 1.327 1.387-.023.76-.578 1.291-1.354 1.291zm3.291-6.531c-.184.26-.588.586-1.098.983l-.562.387c-.308.24-.494.467-.563.688-.056.174-.082.221-.087.576v.09H8.685l.006-.182c.027-.744.045-1.184.354-1.547.485-.568 1.555-1.258 1.6-1.287a1.65 1.65 0 0 0 .379-.387c.225-.311.324-.555.324-.793 0-.334-.098-.643-.293-.916-.188-.266-.545-.398-1.061-.398-.512 0-.863.162-1.072.496-.216.341-.325.7-.325 1.067v.092H6.386l.004-.096c.057-1.353.541-2.328 1.435-2.897.563-.361 1.264-.544 2.081-.544 1.068 0 1.972.26 2.682.772.721.519 1.086 1.297 1.086 2.311-.001.567-.18 1.1-.534 1.585z"/>
                </svg>
                <tooltip transition="fade" :show="showPeoplesPointsTooltip">По данным Народного Судейства <a href="https://www.youtube.com/channel/UCtCoIu-usWXeHe1uaohUkPA" target="_blank" class="text-blue-500">VeksadaS</a></tooltip>
            </div>
            <div class="mt-1">
            ...
            <div class="flex items-center mt-2">
                Рейтинг: &nbsp;
                <svg v-for="i in 5" :key="i" :class="i <= rating ? 'text-teal-500' : 'text-gray-400'" class="w-4 h-4 fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
